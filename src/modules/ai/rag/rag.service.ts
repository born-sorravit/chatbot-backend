...
import { AiAgentKnowledgeBaseEntity } from '@/models/entities';
import {
  EmbeddingsRepository,
  type RetrievedChunk,
} from '@/models/knowledge-base/embeddings.repository';
import { ConfigService } from '@nestjs/config';
import {
  EMBEDDING_PROVIDER,
  type EmbeddingProvider,
} from '@/modules/ai/providers/embedding.provider';
...
export class RagService {
  ...
  get maxDistance(): number {
    return this.config.get<number>('rag.maxDistance') ?? this.provider.defaultMaxDistance;
  }
  ...
  async retrieve(
    ...
    const candidates = await this.embeddings.search(
      ...
      knowledgeBaseIds,
      queryVector,
      this.config.getOrThrow<number>('rag.candidateK'),
    );

    const relevant = candidates
      .filter((chunk) => chunk.distance <= maxDistance)
      .slice(0, this.config.getOrThrow<number>('rag.topK'));
