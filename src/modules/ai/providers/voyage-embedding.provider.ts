...
export class VoyageEmbeddingProvider implements EmbeddingProvider {
  ...
  constructor(private readonly config: ConfigService) {
    this.apiKey = this.config.getOrThrow<string>('embedding.apiKey');
  ...
  get model(): string {
    return this.config.getOrThrow<string>('embedding.model');
  }
  ...
  async embedMany(texts: string[], kind: EmbeddingKind): Promise<number[][]> {
    ...
    const response = await fetch('https://api.voyageai.com/v1/embeddings', {
      ...
      signal: AbortSignal.timeout(this.config.getOrThrow<number>('llm.timeoutMs')),
    });
    ...
    for (const [index, vector] of ordered.entries()) {
      ...
      if (vector.length !== this.dimensions) {
        ...
        throw new Error(`Voyage returned ${vector.length} dimensions, expected ${this.dimensions}`);
      }
    }
