...
import { CustomerSessionEntity } from '../database/entities';
...
export class CustomerSessionService {
  ...
  private ttlMs(): number {
    const match = /^(\d+)([smhd])$/.exec(this.config.customerSessionTtl);

    if (!match) {
      throw new Error(`Invalid CUSTOMER_SESSION_TTL: ${this.config.customerSessionTtl}`);
    }
