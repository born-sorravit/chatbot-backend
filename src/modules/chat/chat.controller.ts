...
import { CurrentCustomer, Public } from '@/shared/decorators';
import {
...
export class ChatController {
  ...
  async createSession(
    ...
    response.cookie(CUSTOMER_SESSION_COOKIE, result.sessionToken, {
      httpOnly: true,
      sameSite: 'lax',
      secure: this.config.getOrThrow<boolean>('app.isProduction'),
  ...
  async send(@CurrentCustomer() customer: AuthenticatedCustomer, @Body() dto: SendMessageDto) {
