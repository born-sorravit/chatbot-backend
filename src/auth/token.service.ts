...
import { RefreshTokenEntity, type UserEntity } from '../database/entities';
...
export class TokenService {
  ...
  async signAccessToken(user: UserEntity): Promise<string> {
    ...
    return this.jwt.signAsync(payload, {
      ...
      expiresIn: Math.floor(this.parseDurationToMs(this.config.jwtAccessTtl) / 1000),
    });
  }
  ...
  async issueRefreshToken(
    ...
    const expiresAt = new Date(Date.now() + this.parseDurationToMs(this.config.jwtRefreshTtl));
